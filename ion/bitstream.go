...
func (b *bitstream) ReadTimestamp() (Timestamp, error) {
	...
	nsecs := 0
	overflow := false
	fractionPrecisionUnits := uint8(0)

	// Check the fractional seconds part of the timestamp.
	if length > 0 {
		nsecs, fractionPrecisionUnits, overflow, err = b.readNsecs(length)
		if err != nil {
			return Timestamp{}, err
		}

		if fractionPrecisionUnits > 0 {
			precision = TimestampPrecisionNanosecond
		}
	}

	timestamp, err := tryCreateTimestamp(ts, nsecs, overflow, offset, osign, precision, fractionPrecisionUnits)
	if err != nil {
		return Timestamp{}, err
	}
...
func (b *bitstream) readNsecs(length uint64) (int, uint8, bool, error) {
	...
	if err != nil || nsec < 0 || nsec > 999999999 {
		...
		return 0, 0, false, &SyntaxError{msg, b.pos}
	}
	...
	// Overflow to second.
	if nsec == 1000000000 {
		return 0, exponent, true, nil
	}

	return int(nsec), exponent, false, nil
}
